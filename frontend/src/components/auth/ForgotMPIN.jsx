...
const ForgotMPIN = () => {
  ...
  const [step, setStep] = useState(1); // 1: Phone, 2: OTP, 3: New MPIN
  ...
  const handlePhoneSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await authAPI.sendOTP(phone, 'mpin-reset', channel);
      if (result.success) {
        setStep(2);
        if (result.message) {
          setError('');
  ...
  const handleOTPSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await authAPI.verifyOTP(phone, otp, 'mpin-reset');
      if (result.success) {
        setStep(3);
      } else {
        const errs = Array.isArray(result.errors) ? result.errors : [];
  ...
  const handleMpinReset = async (e) => {
    e.preventDefault();
    setError('');

    const mpinValidation = validateMPIN(newMpin);
    ...
    setLoading(true);
    try {
      const result = await authAPI.resetMPIN(phone, otp, newMpin, confirmMpin);
      if (result.success) {
        ...
        const errs = Array.isArray(result.errors) ? result.errors : [];
        setError(errs[0] || result.message || 'Failed to reset MPIN. Please try again.');
      }
    } catch (err) {
      setError('An error occurred. Please try again.');
    } finally {
  ...
  const stepTitle =
    step === 1 ? 'Reset MPIN' : step === 2 ? 'Verify OTP' : 'Set New MPIN';
  const stepSubtitle =
    step === 1
      ...
      : step === 2
        ...
        : 'Choose a new 6-digit MPIN for payments and wallet actions';
          ...
          <div className="absolute bottom-8 xl:bottom-12 left-8 xl:left-12 right-8 xl:right-12 space-y-6">
            <div className="text-center">
              <p className="text-cyan-200 text-lg xl:text-xl font-medium tracking-normal">
                Secure MPIN recovery — verify your identity, then set a new MPIN
              </p>
            </div>
          ...
          <div className="space-y-8">
            ...
            {success && (
              <div className="bg-green-50 border-l-4 border-green-500 text-green-700 p-4 rounded-lg flex items-start animate-fadeIn">
                ...
                <div className="ml-3">
                  <p className="font-semibold">MPIN reset successful!</p>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      maxLength={6}
                      className="w-full pl-12 pr-4 py-3.5 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all duration-200 text-base bg-gray-50 focus:bg-white text-center text-2xl font-bold tracking-widest"
                    />
                  </div>
                </div>
