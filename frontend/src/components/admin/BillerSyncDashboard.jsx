...
const BillerSyncDashboard = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl p-4 text-sm text-amber-950 dark:text-amber-200">
        <p className="font-semibold mb-1">Prerequisites</p>
        <p>
          Complete{' '}
          <Link to="/admin/billavenue-settings" className="text-blue-700 dark:text-blue-300 font-medium underline">
            BillAvenue Settings
          </Link>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700 p-6 shadow-sm">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-slate-100 mb-1">Biller sync dashboard</h1>
        ...
        <label className="block text-xs font-medium text-gray-600 dark:text-slate-400 mb-1">Optional: biller IDs</label>
        <textarea
          className="w-full border border-gray-300 dark:border-slate-600 rounded-lg p-3 h-24 text-sm"
        ...
        <button
          type="button"
          disabled={loading}
          className="mt-3 ml-2 px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-sm font-medium rounded-lg border border-slate-300 dark:border-slate-600 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50"
        ...
        <button
          type="button"
          disabled={loading}
          className="mt-3 ml-2 px-5 py-2.5 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-900 dark:text-emerald-300 text-sm font-medium rounded-lg border border-emerald-300 dark:border-emerald-800 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 disabled:opacity-50"
        ...
        {health && (
          <div className="mt-4 text-sm space-y-2">
            <p className={`font-medium ${health.go_live_blocked ? 'text-amber-800 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>
              {health.go_live_blocked ? 'Go-live blocked' : 'Prerequisites look good'}
            </p>
            {health.entitlement_probe_ok === false && (
              <p className="text-red-800 dark:text-red-300 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded p-2">
            ...
            {Array.isArray(health.blockers) && health.blockers.length > 0 && (
              <p className="text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded p-2">
        ...
        {readiness && (
          <div className="mt-4 text-sm space-y-2">
            <p className={`font-medium ${readiness.go_live_blocked ? 'text-amber-800 dark:text-amber-300' : 'text-green-700 dark:text-green-300'}`}>
        ...
        {result && (
          <div className="mt-4">
            <p className="text-sm font-medium text-gray-800 dark:text-slate-200 mb-2">Response</p>
            <pre className="text-xs bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-3 overflow-auto max-h-96">
            ...
            {result.success && result.data && (
              <div className="mt-2 space-y-2 text-sm">
                ...
                {result.data.biller_count === 0 && !result.data.agent_id_used && (
                  <p className="text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg p-2">
                ...
                {result.data.biller_count === 0 && result.data.agent_id_used && (
                  <p className="text-amber-800 dark:text-amber-300">
                    Counts are zero with agent <code className="text-xs bg-slate-100 dark:bg-slate-800 px-1 rounded">{result.data.agent_id_used}</code> — the
                ...
                {String(result.data.upstream_status_code || '') === '205' && (
                  <p className="text-red-800 dark:text-red-300 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded-lg p-2">
                ...
                {result.data.retry_without_agent_used && (
                  <p className="text-blue-800 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 rounded-lg p-2">
            ...
            {result.success === false && (
              <div className="text-sm text-red-700 dark:text-red-300 mt-2 space-y-1">
                ...
                {result.data?.hint && <p className="text-red-900 dark:text-red-300 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded p-2">{result.data.hint}</p>}
                {(result.status === 422 || result.status === 502) && !result.data?.hint && (
                  <p className="text-red-900 dark:text-red-300 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 rounded p-2">
