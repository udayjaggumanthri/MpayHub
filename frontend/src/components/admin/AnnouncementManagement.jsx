...
const CompactRowActions = ({ announcement, onView, onToggleActive, onEdit, onDelete }) => (
  <div className="flex items-center justify-center gap-1 flex-shrink-0">
    <button
      ...
      className="p-1.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-xs font-medium"
    ...
    <button
      ...
      className="p-1.5 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/60 rounded-lg"
      title="Edit"
    >
      <FaPen size={16} />
    </button>
    <button
      ...
      className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide ${
        announcement.is_active
          ? 'text-amber-800 dark:text-amber-300 bg-amber-100 dark:bg-amber-900/40 hover:bg-amber-200'
          : 'text-emerald-800 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-900/40 hover:bg-emerald-200'
      }`}
    ...
    <button
      ...
      className="p-1.5 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/60 rounded-lg"
      title="Delete"
    >
      <FaTrash size={16} />
    </button>
...
const AnnouncementManagement = () => {
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      ...
      default:
        return 'bg-gray-50 dark:bg-slate-800/50 text-gray-800 dark:text-slate-200 border-gray-200 dark:border-slate-700';
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-sm px-4 py-3 border border-gray-200 dark:border-slate-700">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-slate-100">Announcement Management</h1>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
        {/* Compact toolbar */}
        <div className="px-3 py-2 border-b border-gray-100 dark:border-slate-800 bg-gray-50/80 dark:bg-slate-800/50">
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative flex-1 min-w-[160px] max-w-md">
              <FaMagnifyingGlass className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500" size={14} />
              <input
                type="search"
                value={searchInput}
                onChange={(e) => setSearchInput(e.target.value)}
                placeholder="Search title or message…"
                className="w-full pl-8 pr-2 py-1.5 text-sm border border-gray-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <select
              ...
              className="text-sm border border-gray-200 dark:border-slate-700 rounded-md py-1.5 px-2 bg-white dark:bg-slate-900 max-w-[120px]"
              title="Priority"
            >
            ...
            <select
              ...
              className="text-sm border border-gray-200 dark:border-slate-700 rounded-md py-1.5 px-2 bg-white dark:bg-slate-900"
            >
            ...
            <div className="flex rounded-md border border-gray-200 dark:border-slate-700 p-0.5 bg-white dark:bg-slate-900 ml-auto">
              <button
                type="button"
                onClick={() => setViewAndPersist('list')}
                className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-slate-100 dark:bg-slate-800 text-blue-700 dark:text-blue-300' : 'text-gray-500 dark:text-slate-400'}`}
                title="List view"
              >
                <FaList size={15} />
              </button>
              <button
                ...
                className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-slate-100 dark:bg-slate-800 text-blue-700 dark:text-blue-300' : 'text-gray-500 dark:text-slate-400'}`}
                title="Grid view"
              >
          ...
          <button
            ...
            className="mt-1.5 flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400 font-medium hover:text-blue-800 dark:hover:text-blue-200"
          >
          ...
          {filtersExpanded && (
            <div className="mt-2 flex flex-wrap items-end gap-2 pt-2 border-t border-gray-200/80 dark:border-slate-700/80">
              <select
                value={filterRole}
                onChange={(e) => {
                ...
                className="text-sm border border-gray-200 dark:border-slate-700 rounded-md py-1.5 px-2 bg-white dark:bg-slate-900 min-w-[140px]"
              >
              ...
              <button
                ...
                className="text-xs px-2 py-1.5 rounded-md border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 disabled:opacity-40"
              >
                Clear all
              </button>
              <span className="text-[10px] text-gray-400 dark:text-slate-500">UTC date range</span>
            </div>
          )}
        </div>

        <div className="px-3 py-2 flex items-center justify-between gap-2 border-b border-gray-50">
          <h3 className="text-sm font-semibold text-gray-800 dark:text-slate-200">Announcements</h3>
          {!loading && !loadError && (
        ...
        <div className="p-3">
          {loading && <div className="text-center py-8 text-sm text-gray-500 dark:text-slate-400">Loading…</div>}
          {!loading && loadError && (
            <div className="text-center py-6 text-red-600 dark:text-red-400 text-sm">{loadError}</div>
          )}
          ...
          {!loading && !loadError && announcements.length > 0 && viewMode === 'list' && (
            <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-slate-700">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 dark:bg-slate-800/50 text-left text-[10px] uppercase tracking-wider text-gray-500 dark:text-slate-400 border-b border-gray-200 dark:border-slate-700">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                  {announcements.map((a) => (
                    <tr
                      key={a.id}
                      className={`hover:bg-gray-50/80 dark:hover:bg-slate-800/80 ${!a.is_active ? 'bg-amber-50/40 dark:bg-amber-950/40' : ''}`}
                    >
                      <td className="px-2 py-2 align-middle">
                        {a.image_url ? (
                          <button
                            ...
                            className="block w-11 h-11 rounded border border-gray-200 dark:border-slate-700 overflow-hidden bg-gray-50 dark:bg-slate-800/50 focus:ring-2 focus:ring-blue-400"
                            title="View"
                          >
                            <img src={a.image_url} alt="" className="w-full h-full object-cover" />
                          </button>
                        ) : (
                          <div className="w-11 h-11 rounded border border-dashed border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50 flex items-center justify-center text-[9px] text-gray-400 dark:text-slate-500">
                            —
                          </div>
                        )}
                      </td>
                      <td className="px-2 py-2 align-middle">
                        <button
                          type="button"
                          onClick={() => setViewDetailAnnouncement(a)}
                          className="text-left font-medium text-gray-900 dark:text-slate-100 hover:text-blue-700 dark:hover:text-blue-200 line-clamp-2"
                        >
                          {displayTitle(a)}
                        </button>
                        <p className="sm:hidden text-[10px] text-gray-500 dark:text-slate-400 mt-0.5">
                          {formatCreated(a.createdAt)}
                        </p>
                      </td>
                      <td className="px-2 py-2 align-middle text-gray-600 dark:text-slate-400 text-xs whitespace-nowrap hidden sm:table-cell">
                        {formatCreated(a.createdAt)}
                      </td>
                      <td className="px-2 py-2 align-middle">
                        <span
              ...
              {announcements.map((a) => (
                <div
                  ...
                  className={`rounded-lg border p-2 flex flex-col gap-1.5 ${
                    a.is_active ? 'border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900' : 'border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/40'
                  }`}
                >
                  <button
                    type="button"
                    onClick={() => setViewDetailAnnouncement(a)}
                    className="aspect-[4/3] w-full rounded-md border border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50 overflow-hidden flex items-center justify-center focus:ring-2 focus:ring-blue-400"
                  >
                    {a.image_url ? (
                      <img src={a.image_url} alt="" className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <p className="text-xs font-semibold text-gray-900 dark:text-slate-100 line-clamp-2 leading-snug min-h-[2rem]">
                    {displayTitle(a)}
                  </p>
                  ...
                  <span
                    className={`text-[9px] font-bold uppercase tracking-wide w-fit px-1.5 py-0.5 rounded ${
                      a.is_active ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300' : 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-slate-300'
                    }`}
                  >
                    {a.is_active ? 'Active' : 'Off'}
                  </span>
                  <div className="pt-1 border-t border-gray-100 dark:border-slate-800 mt-auto">
                    <CompactRowActions
                      announcement={a}
                      onView={setViewDetailAnnouncement}
                      onToggleActive={handleToggleActive}
          ...
          {!loading && !loadError && totalCount > 0 && (
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-gray-100 dark:border-slate-800">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md border border-gray-200 dark:border-slate-700 text-xs font-medium disabled:opacity-40"
                >
      ...
      {viewDetailAnnouncement && (
        ...
        >
          ...
          >
            <div className="sticky top-0 flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900">
              <h2 id="ann-view-title" className="text-lg font-bold text-gray-900 dark:text-slate-100 pr-4">
                {displayTitle(viewDetailAnnouncement)}
              </h2>
              <button
                type="button"
                onClick={() => setViewDetailAnnouncement(null)}
                className="p-1 rounded-lg text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700"
                aria-label="Close"
              >
                <FiX size={22} />
              </button>
            ...
            <div className="p-4 space-y-3 text-sm">
              <div className="flex flex-wrap gap-2 text-xs">
                ...
                <span className="text-gray-500 dark:text-slate-400">{formatCreated(viewDetailAnnouncement.createdAt)}</span>
              </div>
              {viewDetailAnnouncement.image_url && (
                <div className="rounded-lg border border-gray-100 dark:border-slate-800 overflow-hidden bg-gray-50 dark:bg-slate-800/50">
                  <img
              ...
              {(viewDetailAnnouncement.message || '').trim() ? (
                ...
                <p className="text-gray-400 dark:text-slate-500 italic text-sm">No message text</p>
              )}
              <div>
                <p className="text-xs font-semibold text-gray-500 dark:text-slate-400 mb-1">Target roles</p>
                <div className="flex flex-wrap gap-1">
                  {viewDetailAnnouncement.targetRoles.map((role) => (
                    <span key={role} className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-slate-800 rounded text-gray-700 dark:text-slate-300">
                      {role}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-100 dark:border-slate-800">
                ...
                <button
                  type="button"
                  onClick={() => setViewDetailAnnouncement(null)}
                  className="px-3 py-2 text-sm font-medium rounded-lg border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800"
                >
                  Close
                </button>
              </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                    Title <span className="text-gray-400 dark:text-slate-500 font-normal">(optional)</span>
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                    Message{' '}
                    <span className="text-gray-400 dark:text-slate-500 font-normal">(optional if image is set)</span>
                  </label>
                  <textarea
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                    Image <span className="text-gray-400 dark:text-slate-500 font-normal">(optional)</span>
                  </label>
                  ...
                  <input
                    ...
                    className="block w-full text-sm text-gray-600 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                  />
                  {imagePreview && (
                    <div className="mt-3 relative inline-block">
                      <img
                        src={imagePreview}
                        alt="Preview"
                        className="max-h-40 rounded-lg border border-gray-200 dark:border-slate-700 object-contain bg-gray-50 dark:bg-slate-800/50"
                      />
                      <button
                        type="button"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {availableRoles.map((role) => (
                      <label
                        key={role}
                        className="flex items-center space-x-2 p-3 border border-gray-300 dark:border-slate-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors"
                      >
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className="text-sm text-gray-700 dark:text-slate-300">{role}</span>
                      </label>
                    ))}
                  </div>
              ...
              <div className="flex items-center justify-end space-x-3 mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
                <button
                  ...
                  className="px-4 py-2 text-gray-700 dark:text-slate-300 bg-gray-100 dark:bg-slate-800 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
